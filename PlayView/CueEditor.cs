...
namespace PolarBearsProgram
{
    public partial class Cue_Editor : Form
    {
        ...
        public void DoWork()
        {
            ...
            {
                 setPointVelocity.GetSetValue = acceleration;
            }
            ...
            int howLong;
            int currentDegrees = Program.playView.position.turnedDegrees;
            int timeBefore = DateTime.Now.Second;
            ...
            Console.WriteLine("timebefore " + timeBefore);
            if (rotation == 0)
            {
        ...
        public Boolean ValidateLimits(int Rotation, int Acceleration, int Deceleration, int Velocity)
        {
           ...
           // if (valRotationSoft != 0 && valRotationSoft > int.Parse(textBoxCW.Text))// && !rotationSoftLimitMessagePoppedUp)
          //  {
            //    MessageBox.Show("You can enter values greater than the Soft Limit, but a soft limit was set for a reason. Please consider using smaller values for rotation.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
               //rotationSoftLimitMessagePoppedUp = true;
         //   }
