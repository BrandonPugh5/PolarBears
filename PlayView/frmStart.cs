...
{/*
	public class frmStart : System.Windows.Forms.Form
	{
        ...
        public void DoWork()
        {
            ...
            if (string.IsNullOrWhiteSpace(accelerationTextBox.Text) || !float.TryParse(accelerationTextBox.Text, out f) || Convert.ToInt16(accelerationTextBox.Text) == 0 || acceleration == 0)
            {
                MessageBox.Show("Must input a non-zero decimal value for acceleration!");
            }
            else
            {
                acceleration = Convert.ToInt16(accelerationTextBox.Text);
                //logForm.AddRow("Changed acceleration to " + accelerationTextBox.Text, "", "", 0, setPointVelocity.GetSetValue);
            }
            if (string.IsNullOrWhiteSpace(velocityTextBox.Text) || !float.TryParse(velocityTextBox.Text, out f))
            {
